  ...
  useEffect(() => {
    const fetchTreatments = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3001/api/treatments/details/${patient_id}`, {
          headers: {
        ...
        const groupedTreatments = response.data.reduce((acc, treatment) => {
          const existingTreatment = acc.find(item => item.treatment_description === treatment.treatment_description);
            ...
            <Grid container spacing={3}>
              {treatments.length > 0 ? (
                treatments.map((treatment) => (
                  <Grid item xs={12} sm={6} md={4} key={treatment.treatment_id}>
                    <Card>
                      <CardContent>
